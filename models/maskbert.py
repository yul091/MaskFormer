...
class BertModel(BertPreTrainedModel):
    def __init__(self, config, add_pooling_layer=True, mask_rate=0):
        super().__init__(config)
        self.config = config

        self.embeddings = BertEmbeddings(config)
        self.encoder = BertEncoder(config, mask_rate=mask_rate)
...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config, mask_rate=0):
        ...
        self.bert = BertModel(config, mask_rate=mask_rate)
        classifier_dropout = (
...
if __name__ == '__main__':
    ...
    for mask_rate in np.arange(0, 1.1, 0.1):
        ...
        logging.info(f'Mask rate: {mask_rate}, total time: {end - start:.4f} s, avg time: {(end - start) / 1000:.4f} s')
